            ...
            [distribution, { node {
              try {
                ...
                retry(params.retries as Integer) {
                  ...
                  archiveArtifacts artifacts: "$debian_dir/Dockerfile*, $graphs_dir/*, packages.txt", allowEmptyArchive: true
                ...
                // bundle_image.inside("-v $HOME/tailor/ccache:/ccache -e CCACHE_DIR=/ccache -e CCACHE_MAXSIZE=4900M") {
                  ...
                  if (colcon_cache_enabled){
                    ...
                    string(credentialsId: 'tailor_restic_password', variable: 'RESTIC_PASSWORD'),
                      ...
                      // Store
                      sh("""
                        restic -r ${restic_repo} backup $build_dir $install_dir --tag ${cacheTag(distribution, params.release_label)} --retry-lock 1m || true
                      """)
                    }
                  }
                  else{
