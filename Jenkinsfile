...
def uploadSuccessfulBuildParams = {
  ...
  def latestFile = 'tailor-distro-successful-params.yaml'
  def historyFile = "tailor-distro-successful-params-${timestampSuffix}.yaml"
  def payload = [
    pipeline: 'tailor-distro',
  ...
  withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'tailor_aws']]) {
    ...
    s3Upload(
      bucket: bucketName,
      path: "${params.release_label}/pipeline-params/tailor-distro",
      includePathPattern: latestFile,
      workingDir: '.'
    )
    s3Upload(
      bucket: bucketName,
      path: "${params.release_label}/pipeline-params/tailor-distro/history",
